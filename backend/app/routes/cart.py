...
def get_cart_data(cart_data: Dict[int, int], db: Session = Depends(get_db)):
    service = CartService(db)
    return service.get_cart_details(cart_data)


@router.post("/update", status_code=status.HTTP_200_OK)
def update_cart_data(request: UpdateCartRequest, db: Session = Depends(get_db)):
    service = CartService(db)
    item = CartItemUpdate(product_id=request.product_id, amount=request.amount)
    updated_cart = service.update_cart_data(request.cart, item)
    return {"cart": updated_cart}
