...
class CartService:
    ...
    def add_to_cart(self, cart_data: Dict[int, int], item: CartItemCreate) -> Dict[int, int]:
        ...
        if item.product_id in cart_data:
            cart_data[item.product_id] += item.amount
        else:
            cart_data[item.product_id] = item.amount

        return cart_data
    
    def update_cart_data(self, cart_data: Dict[int, int], item: CartItemUpdate) -> Dict[int, int]:
        if item.product_id in cart_data:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=f"Product with id {item.product_id} not found"
        ...
        for product_id, amount in cart_data.items():
            if product_id in products_dict:
                ...
                cart_item = CartItem(product_id=product.id, name=product.name, price=product.price, 
                                     amount=amount, subtotal=subtotal, image_url=product.image_url)
